...
# ------------------------------------------------------------------------------
# 2. VSE II TRANSMITTER (Adaptive Step-Size Keying & -2.0V EOS Marker)
# ------------------------------------------------------------------------------
def vse_ii_adaptive_transmitter(data_bytes, step_size):
    ...
    while i < len(data_bytes):
        ...
        # Ha csonka volt az utolsó blokk, kilőjük a 3 darab -2.0V-os zárótüskét
        if is_truncated:
...
def vse_ii_adaptive_receiver(rx_stream, current_step_size):
    ...
    for item in rx_stream:
        # Érzékeljük a -2.0V-os EOS_MARKER-t a stream lezárásához
        if isinstance(item, tuple) and item[0] == "EOS_MARKER":
            for byte in current_block:
                if byte is not None:
                    recovered_buffer.append(byte)
            break
            
        if isinstance(item, tuple) and item[0] == "BLOCK_END":
            total_blocks += 1
            expected_sum = item[1]
            ...
            if missing_count > 0 or actual_sum != expected_sum:
                next_index = rx_stream.index(item) + 1
                # Ha a következő fizikai elem az EOS_MARKER, nem piszkáljuk a csonka blokkot hibajelzőkkel
                if next_index < len(rx_stream) and isinstance(rx_stream[next_index], tuple) and rx_stream[next_index][0] == "EOS_MARKER":
                    pass
                else:
                    corrupted_blocks += 1
                    for idx in range(4):
                        if current_block[idx] is None: 
                            current_block[idx] = ord('?')
    ...
    return bytes(recovered_buffer), bler, next_step_size
...
out_1, bler_1, recommended_step = vse_ii_adaptive_receiver(rx_1, INITIAL_STEP)
...
out_2, bler_2, _ = vse_ii_adaptive_receiver(rx_2, recommended_step)
