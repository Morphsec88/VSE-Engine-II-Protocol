...
#  Core Simulation Engine - Sender & Receiver WITH ENGLISH EFFICIENCY REPORT
#
...
# Traditional Serial: 1 byte = 8 data bits + 1 Start bit + 1 Stop bit = 10 pulses
...
print(f"Processed valid data:       {processed_bytes} bytes")
print(f"Traditional serial pulses:  {traditional_pulses} signals required (10 pulses/byte)")
print(f"VSE II physical data pulses: {vse_data_pulses} pulses used (Voltage-multiplexed)")
print(f"VSE II embedded sync pulses:{vse_sync_pulses} pulses used")
print(f"Total emitted VSE signals:  {vse_total_pulses} pulses")
print(f"👉 WIRE LOAD REDUCTION:      {pulse_saving_percentage:.2f}% fewer signals on the line!")
print("-" * 75)
